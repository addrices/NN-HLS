...
#pragma HLS DATAFLOW
	...
	P2:MaxPool_IOP<P2_PSIZE,ABIT,P2_SIZE,P2_SIZE,P2_CHANNEL,P2_INP,P2_OUTP>(C2_out,P2_out,reps);
	...
	P3:MaxPool_IOP<P3_PSIZE,ABIT,P3_SIZE,P3_SIZE,P3_CHANNEL,P3_INP,P3_OUTP>(C3_out,P3_out,reps);
	...
	P4:MaxPool_IOP_T<P4_PSIZE,ABIT,P4_SIZE,P4_SIZE,P4_CHANNEL,P4_INP,P4_OUTP>(C4_out,P4_out,reps);
	//splitStream_Length<ABIT*P4_CHANNEL,ABIT*F5_INP,1>(P4_out,F5_in);

//	ExtendStreamWidth_Length<ABIT*P4_OUTP,128,8>(P4_out,tout);
//	AddLast<8>(tout,out);

	F5:FcnnLayer_ScaleBit<F5_INP,F5_OUTP,F5_IN,F5_OUT,WBIT,ABIT>(P4_out,F5_out,F5_W,F5_B,F5_SCALEBIT,reps);
	F6:FcnnLayer_ScaleBit<F6_INP,F6_OUTP,F6_IN,F6_OUT,WBIT,ABIT>(F5_out,F6_out,F6_W,F6_B,F6_SCALEBIT,reps);
