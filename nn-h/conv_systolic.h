...
void Orbital_Gemm(ap_int<ABit> weight[WSize][DEPTH],ap_uint<WBit> activation[ASize][Depth],ap_int<MBit> o[ASize][WSize]){
	ap_int<WBit> W[WSize][Depth];
	ap_uint<ABit> A[ASize][Depth];
	ap_int<MBit> O[ASize][WSize];

	W[a][b] = weight[a][(a+b)%Depth];
